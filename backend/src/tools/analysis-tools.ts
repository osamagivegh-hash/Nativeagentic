      ...
      switch (analysisType) {
        ...
        case 'distribution': {
          if (dataType === 'financial') {
            ...
            dataPoints = distResult.rows.reduce((sum: number, r: any) => sum + parseInt(r.count), 0) as any;

            const largeCount = distResult.rows.find((r: any) => r.bucket === 'very_large');
            if (largeCount && parseInt((largeCount as any).count) > dataPoints * 0.1) {
        ...
        case 'segmentation': {
          if (dataType === 'user') {
            ...
            dataPoints = segmentResult.rows.reduce((sum: number, r: any) => sum + parseInt(r.user_count), 0) as any;
