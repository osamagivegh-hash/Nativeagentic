...
async function setupMiddleware(): Promise<void> {
  ...
  await app.register(cors, {
    origin: serverConfig.nodeEnv === 'production'
      ? ['https://nativeagentic.vercel.app', 'http://localhost:5173']
      : true,
    credentials: true
  });
  ...
  // Authentication decorator (simplified for demo) - check if not already added by plugins
  if (!app.hasRequestDecorator('user')) {
    app.decorateRequest('user', null);
  }
  if (!app.hasRequestDecorator('sessionId')) {
